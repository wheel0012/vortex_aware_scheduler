...
# ------------------------------------------------------------
# Paper-like L1D starting point: 32KB, 8-way. For a more stressful
# Vortex sweep, override COMMON_CONFIGS with e.g.
#   COMMON_CONFIGS="-DDCACHE_SIZE=8192 -DDCACHE_NUM_WAYS=2"
# ------------------------------------------------------------
...
# Benchmark groups
# ------------------------------------------------------------
# HCS follows the CCWS paper's spirit: irregular/cache-sensitive
# workloads available in this repository. The original paper also used
# Memcached and Garbage Collection, which are not present here.
#
# CI contains regular/coalesced or compute-heavy kernels that should be
# less favorable to CCWS.
# ------------------------------------------------------------
...
run_ccws_for_bench() {
  ...
  # Paper-style dynamic LLDS + cumulative cutoff. No extra hard cap.
  run_one \
    "${group}" \
    "${bench}" \
    "CCWS" \
  ...
  # Paper-style dynamic LLDS with an 8-warp hard cap, mirroring the
  # paper's K_THROTTLE=8 intuition while keeping Vortex runs bounded.
  run_one \
    "${group}" \
    "${bench}" \
    "CCWS" \
  ...
  # Vortex-tuned aggressive cap for workloads where unrestricted dynamic
  # mode is too weak to protect cache locality.
  run_one \
    "${group}" \
    "${bench}" \
    "CCWS" \
