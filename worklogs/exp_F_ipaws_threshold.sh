#!/usr/bin/env bash
# Experiment F: sweep the iPAWS concave-threshold parameter.
# Holds scheduler=iPAWS, dcache 8-way, CACP enabled at 50% reservation.
# Varies VORTEX_IPAWS_CONCAVE_TH so we can see how the gCAWS<->RR decision
# shifts on each benchmark.

set -u
set -o pipefail

# Accept "bench [args...]" entries.  See exp_E_cacp_reserved.sh.
BENCH_ENTRIES=("$@")
...
BASE_FLAGS="-DDCACHE_NUM_WAYS=8 -DNUM_CORES=1 -DNUM_WARPS=16 -DNUM_THREADS=16 -DPERF_ENABLE -DVORTEX_SCHED=4 -DVORTEX_CACP_ENABLE=1"
...
LOG_ROOT="$ROOT_DIR/worklogs/exp_F_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_ROOT"
...
for combo in "${COMBOS[@]}"; do
  read -r label th <<<"$combo"
  conf="$BASE_FLAGS -DVORTEX_IPAWS_CONCAVE_TH=$th"
...
for entry in "${BENCH_ENTRIES[@]}"; do
  ...
  {
    printf "\n=== %s (exp-F, iPAWS, varying concave threshold) ===\n" "$entry"
    printf "%-7s | %-9s | %-12s | %-12s | %-8s\n" \
      "label" "cycles" "IPC" "dc_read_hit" "dc_wr_hit"
    printf '%s\n' "$(printf '%.0s-' {1..70})"
    for combo in "${COMBOS[@]}"; do
      read -r label th <<<"$combo"
      log="$LOG_ROOT/${label}/${bench}.log"
      [ -f "$log" ] || { printf "%-7s | (no log)\n" "$label"; continue; }
