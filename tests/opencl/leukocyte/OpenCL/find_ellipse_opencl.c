...
float *GICOV_OpenCL(int grad_m, int grad_n, float *host_grad_x, float *host_grad_y) {
	...
	device_gicov = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, grad_mem_size, host_gicov, &error);
	check_error(error, __FILE__, __LINE__);
	...
	size_t device_work_group_size = 0;
	clGetKernelWorkGroupInfo(GICOV_kernel, device, CL_KERNEL_WORK_GROUP_SIZE,
	                         sizeof(kernel_work_group_size), &kernel_work_group_size, NULL);
	clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_GROUP_SIZE,
	                sizeof(device_work_group_size), &device_work_group_size, NULL);
	...
	printf("Find: local_work_size = %zu, global_work_size = %zu, kernel_wg_limit = %zu, device_wg_limit = %zu\n",
	       work_group_size, global_work_size, kernel_work_group_size, device_work_group_size);
	fflush(stdout);
	...
	error = clEnqueueReadBuffer(command_queue, device_gicov, CL_TRUE, 0, grad_mem_size, host_gicov, 0, NULL, NULL);
	check_error(error, __FILE__, __LINE__);
