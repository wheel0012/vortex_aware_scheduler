...
{ 
	...
	// Vortex simx can stall on this benchmark's original tiled kernel because
	// it relies on workgroup-local barriers. Run one stencil step per launch.
	num_iterations = 1;
	
	// Determine GPU work group grid
	size_t global_work_size[2];
	global_work_size[0] = BLOCK_SIZE * ((col + BLOCK_SIZE - 1) / BLOCK_SIZE);
	global_work_size[1] = BLOCK_SIZE * ((row + BLOCK_SIZE - 1) / BLOCK_SIZE);
...
int main(int argc, char** argv) {
  ...
  printf("WG size of kernel = %d X %d\n", BLOCK_SIZE, BLOCK_SIZE);
    ...
    int borderCols = 0;
    int borderRows = 0;
    int blockCols = (grid_cols + BLOCK_SIZE - 1) / BLOCK_SIZE;
    int blockRows = (grid_rows + BLOCK_SIZE - 1) / BLOCK_SIZE;
	...
	if (error != CL_SUCCESS) fatal_CL(error, __LINE__);
	error = clEnqueueWriteBuffer(command_queue, MatrixTemp[0], CL_TRUE, 0, sizeof(float) * size, FilesavingTemp, 0, NULL, NULL);
	if (error != CL_SUCCESS) fatal_CL(error, __LINE__);
	...
	if (error != CL_SUCCESS) fatal_CL(error, __LINE__);
	error = clEnqueueWriteBuffer(command_queue, MatrixPower, CL_TRUE, 0, sizeof(float) * size, FilesavingPower, 0, NULL, NULL);
	if (error != CL_SUCCESS) fatal_CL(error, __LINE__);
	/*
	printf("grid=%d x %d\n", grid_rows, grid_cols);
	printf("BLOCK_SIZE=%d, pyramid_height=%d, EXPAND_RATE=%d\n",
		BLOCK_SIZE, pyramid_height, EXPAND_RATE);
	printf("blockRows=%d, blockCols=%d\n", blockRows, blockCols);
	printf("global_work_size=%zu x %zu\n",
		global_work_size[0], global_work_size[1]);
	printf("local_work_size=%zu x %zu\n",
	...
	cl_float *MatrixOut = (cl_float *) malloc(sizeof(float) * size);
	if (!MatrixOut) fatal("unable to allocate output memory");
	error = clEnqueueReadBuffer(command_queue, MatrixTemp[ret], CL_TRUE, 0, sizeof(float) * size, MatrixOut, 0, NULL, NULL);
	if (error != CL_SUCCESS) fatal_CL(error, __LINE__);
	...
	// Make sure all queued OpenCL work is completed before teardown
	error = clFinish(command_queue);
	if (error != CL_SUCCESS) fatal_CL(error, __LINE__);

	// Release memory objects
	error = clReleaseMemObject(MatrixTemp[0]);
	if (error != CL_SUCCESS) fatal_CL(error, __LINE__);

	error = clReleaseMemObject(MatrixTemp[1]);
	if (error != CL_SUCCESS) fatal_CL(error, __LINE__);

	error = clReleaseMemObject(MatrixPower);
	if (error != CL_SUCCESS) fatal_CL(error, __LINE__);

	// Release kernel/program/queue/context
