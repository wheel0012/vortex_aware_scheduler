...
int _clClampWorkGroupSize(int requested) {
	size_t max_work_group_size = requested;
	cl_int status = clGetDeviceInfo(oclHandles.devices[device_id_inused],
	                                CL_DEVICE_MAX_WORK_GROUP_SIZE,
	                                sizeof(max_work_group_size),
	                                &max_work_group_size,
	                                NULL);
	if (status == CL_SUCCESS && requested > (int)max_work_group_size)
		return (int)max_work_group_size;
	return requested;
}
...
void _clRelease()
{
    ...
    if (oclHandles.queue != NULL) {
        cl_int resultCL = clFinish(oclHandles.queue);
        if (resultCL != CL_SUCCESS) {
            cerr << "ReleaseCL()::Error: In clFinish" << endl;
            errorFlag = true;
        }
    }
    ...
    if (oclHandles.devices != NULL){
        cl_int resultCL = clReleaseDevice(oclHandles.devices[device_id_inused]);
        if (resultCL != CL_SUCCESS){
...
void* _clMallocHost(int size)throw(string){
	void *mem_h = malloc(size);
	if (mem_h == NULL)
		throw(string("exception in _clMallocHost -> malloc"));
	return mem_h;
}
/*------------------------------------------------------------
...
void _clInvokeKernel(int kernel_id, int work_items, int work_group_size) throw(string){
	...
	cl_event e[1];
	work_group_size = _clClampWorkGroupSize(work_group_size);
...
void _clMemset(cl_mem mem_d, short val, int number_bytes)throw(string){
	...
	cl_event e[1];
	int local_size = _clClampWorkGroupSize(work_group_size);
	if(work_items%local_size != 0)	//process situations that work_items cannot be divided by work_group_size
	  work_items = work_items + (local_size-(work_items%local_size));
  	size_t local_work_size[] = {local_size, 1};
	size_t global_work_size[] = {work_items, 1};
	oclHandles.cl_status = clEnqueueNDRangeKernel(oclHandles.queue, oclHandles.kernel[kernel_id], work_dim, 0, \
											global_work_size, local_work_size, 0 , 0, &(e[0]) );	
#ifdef ERRMSG
