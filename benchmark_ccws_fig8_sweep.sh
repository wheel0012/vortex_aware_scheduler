...
# ============================================================
# CCWS Figure-8-style MPKI sensitivity sweep
#
# Focus benchmarks:
#   bfs, kmeans, sgemm3
#
# The goal is to inspect whether CCWS changes L1D MPKI before
# judging speedup. Summary rows include dcache MPKI, memory latency,
# and CCWS intervention counters.
#
# Usage:
#   chmod +x benchmark_ccws_fig8_sweep.sh
#   ./benchmark_ccws_fig8_sweep.sh
#
# Useful overrides:
#   BENCHMARKS="bfs kmeans" ./benchmark_ccws_fig8_sweep.sh
#   CACHE_NAMES="c8k_w2 c16k_w4" CACHE_SIZES="8192 16384" CACHE_WAYS="2 4" ./benchmark_ccws_fig8_sweep.sh
#   CCWS_KS="8 16 32" CCWS_VTAS="16 32" CCWS_CAPS="0 2 4" ./benchmark_ccws_fig8_sweep.sh
#   CORES=1 WARPS=32 THREADS=16 ./benchmark_ccws_fig8_sweep.sh
# ============================================================
...
# Figure-8-like cache sweep. 4KB/1-way can expose more MPKI, but may
# hit simulator corner cases on some workloads, so keep it opt-in.
if [[ "${INCLUDE_TINY_CACHE:-0}" == "1" ]]; then
  read -r -a CACHE_NAMES_ARR <<< "${CACHE_NAMES:-c4k_w1 c8k_w2 c16k_w4 c32k_w8}"
  read -r -a CACHE_SIZES_ARR <<< "${CACHE_SIZES:-4096 8192 16384 32768}"
  read -r -a CACHE_WAYS_ARR  <<< "${CACHE_WAYS:-1 2 4 8}"
else
  read -r -a CACHE_NAMES_ARR <<< "${CACHE_NAMES:-c8k_w2 c16k_w4 c32k_w8}"
  read -r -a CACHE_SIZES_ARR <<< "${CACHE_SIZES:-8192 16384 32768}"
  read -r -a CACHE_WAYS_ARR  <<< "${CACHE_WAYS:-2 4 8}"
fi
...
read -r -a CCWS_K_ARR    <<< "${CCWS_KS:-8 16}"
read -r -a CCWS_VTA_ARR  <<< "${CCWS_VTAS:-16 32}"
read -r -a CCWS_CAP_ARR  <<< "${CCWS_CAPS:-0 4}"
...
run_one() {
  ...
  local log_dir="${OUT_DIR}/${bench}/${cache_name}"
  mkdir -p "${log_dir}"
  ...
  summarize_log "${bench}" "${policy}" "${config_name}" "${cache_name}" "${cache_size}" "${cache_ways}" "${status}" "${log_file}"
  echo
}
...
echo "============================================================"
echo "Starting CCWS Figure-8-style MPKI sweep"
...
echo "============================================================"
echo "Figure-8-style sweep complete"
